...
// 默认图标组件（与 Sidebar 中的 DefaultBoardIcon 保持一致）
const DefaultBoardIcon: React.FC<{ size: number; className?: string }> = ({ size, className }) => (
  <svg
...
const BoardIcon: React.FC<BoardIconProps> = ({ 
  ...
  // 使用 useMemo 缓存 URL，只在 board.icon 变化时才重新生成
  // 避免每次渲染都生成新时间戳导致无限重渲染
  const imageUrl = useMemo(() => {
    if (!board.icon) return '';
    const normalized = board.icon.replace(/\\/g, '/');
    return 'local-media:///' + encodeURI(normalized);
  }, [board.icon]);

  // 没有图标或加载失败时显示默认图标
  if (!board.icon || imageError) {
    return <DefaultBoardIcon size={pixelSize} className={className} />;
  }
