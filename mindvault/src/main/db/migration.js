...
export function getDatabasePath(userDataPath) {
    var dbDir = path.join(userDataPath, 'mindvault');
    ensureDir(dbDir);
 ...
 * @param userDataPath - Electron用户数据目录
 */
export function getMediaDir(userDataPath) {
    var mediaDir = path.join(userDataPath, 'mindvault', 'media');
    ensureDir(mediaDir);
    return mediaDir;
}
/**
 ...
 * @param userDataPath - Electron用户数据目录
 */
export function getBackupDir(userDataPath) {
    var backupDir = path.join(userDataPath, 'mindvault', 'backups');
    ensureDir(backupDir);
    return backupDir;
}
/**
...
export function initDatabase(userDataPath) {
    var dbPath = getDatabasePath(userDataPath);
    // 创建/打开数据库
    ...
    if (fs.existsSync(schemaPath)) {
        var schema = readFileSync(schemaPath, 'utf-8');
        db.exec(schema);
    ...
    try {
        ...
        // 先检查外键是否需要修复
        var hasBadForeignKey = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='board_canvas_items'")
            .get();
        if (hasBadForeignKey) {
            var sql = hasBadForeignKey.sql;
    ...
    try {
        var canvasColumns = db.prepare("PRAGMA table_info(board_canvas_items)").all();
        var canvasColumnNames = canvasColumns.map(function (c) { return c.name; });
    ...
    try {
        var stickyColumns = db.prepare("PRAGMA table_info(board_sticky_notes)").all();
        var stickyColumnNames = stickyColumns.map(function (c) { return c.name; });
    ...
    try {
        var edgeCols = db.prepare("PRAGMA table_info(board_canvas_edges)").all().map(function (c) { return c.name; });
    ...
    // 迁移：创建 trash_items 表
    try {
        var tableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='trash_items'").get();
        if (!tableExists) {
    ...
    try {
        var boardCols = db.prepare("PRAGMA table_info(boards)").all().map(function (c) { return c.name; });
